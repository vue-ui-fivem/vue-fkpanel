<template>
  <div id="container">
    <nav id="navBar">
      ...
      <div id="navbarItems">
        <div
          class="navbarItemsDiv"
          :class="this.$route.path == '/employee' ? 'activeBorder' : ''"
        >
          <p @click="$router.push('/employee')" class="itemP">
            <i class="fa-solid fa-person"></i>Alkalmazottak
          </p>
        </div>
        <div
          class="navbarItemsDiv"
          :class="this.$route.path == '/storage' ? 'activeBorder' : ''"
        >
          <p @click="$router.push('/storage')" class="itemP">
            <i class="fa-solid fa-box"></i>Raktár
          </p>
        </div>
        <div
          class="navbarItemsDiv"
          :class="this.$route.path == '/cars' ? 'activeBorder' : ''"
        >
          <p @click="$router.push('/cars')" class="itemP">
            <i class="fa-solid fa-car"></i>Járművek
          </p>
        </div>
        <div
          class="navbarItemsDiv"
          :class="this.$route.path == '/settings' ? 'activeBorder' : ''"
        >
          <p @click="$router.push('/settings')" class="itemP" id="settings">
            <i class="fa-solid fa-screwdriver-wrench"></i>Beállítások
          </p>
        </div>
        <p
          style="
...
export default {
  ...
  mounted() {
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data.type == "ui") {
        ...
        this.$store.state.canBeOrderStorage = event.data.orderableItems;
        this.societyMoney = event.data.factionMoney
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
  ...
  created() {
    this.societyMoney = this.societyMoney
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
    let itemCount = this.$store.state.exitsStorage;
    this.$store.state.exitsStorage = [];
    for (let i = 0; i < itemCount.length; i++) {
      const itemIndex = this.$store.state.exitsStorage.findIndex(
        (el) => el.name == itemCount[i].name
      );
      if (itemIndex != -1) {
